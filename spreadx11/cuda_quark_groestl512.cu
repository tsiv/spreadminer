...
__global__ void __launch_bounds__(256, 4)
 quark_groestl512c_gpu_hash_64_quad(int threads, uint32_t startNounce, uint32_t *g_hash, uint32_t *g_nonceVector)
{
    // durch 4 dividieren, weil jeweils 4 Threads zusammen ein Hash berechnen
    int thread = (blockDim.x * blockIdx.x + threadIdx.x) >> 2;
    if (thread < threads)
...
#pragma unroll 4
        for(int k=0;k<4;k++) message[k] = ((uint32_t *)g_hash)[threads*((k<<2) + (threadIdx.x&0x03))+thread];
#pragma unroll 4
        for(int k=4;k<8;k++) message[k] = 0;
        ...
        {
#pragma unroll 16
            for(int k=0;k<16;k++) ((uint32_t *)g_hash)[k*threads+thread] = hash[k];
        }
    }
}
...
__host__ void quark_groestl512c_cpu_hash_64(int thr_id, int threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order)
{
    int threadsperblock = 256;
    ...
    quark_groestl512c_gpu_hash_64_quad<<<grid, block, shared_size>>>(threads, startNounce, d_hash, d_nonceVector);
