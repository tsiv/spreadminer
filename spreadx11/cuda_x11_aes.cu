...
static __device__ __forceinline__ void aes_gpu_init_128(uint32_t *sharedMemory)
{
	if(threadIdx.x < 128)
	{
        int i = threadIdx.x << 1;
		sharedMemory[i] = d_AES0[i];
		sharedMemory[i+1] = d_AES0[i+1];
		sharedMemory[i+256] = d_AES1[i];
		sharedMemory[i+1+256] = d_AES1[i+1];
		sharedMemory[i+512] = d_AES2[i];
		sharedMemory[i+1+512] = d_AES2[i+1];
		sharedMemory[i+768] = d_AES3[i];
		sharedMemory[i+1+768] = d_AES3[i+1];
	}
}

static __device__ __forceinline__ void aes_gpu_init_64(uint32_t *sharedMemory)
{
	if(threadIdx.x < 64)
	{
        int i = threadIdx.x << 2;
        ...
        sharedMemory[i] = d_AES0[i];
		sharedMemory[i+1] = d_AES0[i+1];
		sharedMemory[i+2] = d_AES0[i+2];
		sharedMemory[i+3] = d_AES0[i+3];
		
        sharedMemory[i+256] = d_AES1[i];
		sharedMemory[i+1+256] = d_AES1[i+1];
		sharedMemory[i+2+256] = d_AES1[i+2];
		sharedMemory[i+3+256] = d_AES1[i+3];
		
        sharedMemory[i+512] = d_AES2[i];
		sharedMemory[i+1+512] = d_AES2[i+1];
		sharedMemory[i+2+512] = d_AES2[i+2];
		sharedMemory[i+3+512] = d_AES2[i+3];
		
        sharedMemory[i+768] = d_AES3[i];
		sharedMemory[i+1+768] = d_AES3[i+1];
		sharedMemory[i+2+768] = d_AES3[i+2];
		sharedMemory[i+3+768] = d_AES3[i+3];
	}
}
