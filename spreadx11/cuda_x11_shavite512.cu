...
c512(const uint32_t* sharedMemory, uint32_t *state, uint32_t *msg, const uint32_t counter)
{
...
__global__ void x11_shavite512c_gpu_hash_64(int threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector)
{
	__shared__ uint32_t sharedMemory[1024];
	aes_gpu_init_128(sharedMemory);
    __syncthreads();
...
#pragma unroll 16
		for(int i=0;i<16;i++)
			msg[i] = ((uint32_t *)g_hash)[i*threads+thread];			
...
__host__ void x11_shavite512c_cpu_hash_64(int thr_id, int threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order)
{
    const int threadsperblock = 128;
    ...
    x11_shavite512c_gpu_hash_64<<<grid, block, shared_size>>>(threads, startNounce, (uint64_t*)d_hash, d_nonceVector);
